  ...
  try {
    ...
    if (p.pubType === 'book') {
      ...
      item = {
        ...
        DOI: p.doi || '',
        // Ces champs existent parfois selon les item types / styles :
        publisher: p.publisher || '',
        place: p.place || '',
        language: p.language || '',
        extra: p.extra || ''
      ...
      // ✅ Nouveau type : Conference Paper
      item = {
        itemType: 'conferencePaper',
        title: p.title,
        creators,

        // Champs Zotero (conferencePaper)
        ...
        // Tout le bloc tex.* + DLAB est dans extra (déjà construit côté front)
        extra: p.extra || ''
      };
